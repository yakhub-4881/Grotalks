...
const MyJourney = () => {
  ...
  const sessions: JourneySession[] = [
    {
      ...
      followUps: [
        { title: 'Update LinkedIn headline', status: 'done', evidence: 'linkedin-screenshot.png' },
        { title: 'Complete a PM case study', status: 'in_progress' },
        { title: 'Reach out to 3 PMs', status: 'done', evidence: 'email-screenshot.png' },
      ],
    ...
    },
    {
      ...
      followUps: [
        { title: 'Draft a 1-page pitch', status: 'done', evidence: 'pitch-deck.pdf' },
        { title: 'Research 5 VCs', status: 'done' },
        { title: 'Build an MVP prototype', status: 'done', evidence: 'mvp-demo-link.txt' },
      ],
  ...
  const totalActions = sessions.reduce((s, sess) => s + sess.followUps.length, 0);
  const doneActions = sessions.reduce((s, sess) => s + sess.followUps.filter(f => f.status === 'done').length, 0);
      ...
      <div className="min-h-[calc(100vh-4rem)] bg-muted py-4 md:py-8">
        <div className="container max-w-3xl px-4">
          ...
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground">Your personal career diary — see how far you've come</p>
          </div>
          ...
          <div className="relative">
            ...
            <div className="space-y-8">
              {sessions.map((session) => {
                ...
                const sessionDone = session.followUps.filter(f => f.status === 'done').length;
                const sessionPct = Math.round((sessionDone / session.followUps.length) * 100);
                        ...
                        {session.followUps.length > 0 && (
                          <div className="space-y-2">
                            <div className="flex items-center justify-between">
                              <p className="text-xs text-muted-foreground">Follow-up Progress</p>
                              ...
                              {session.followUps.map((fu, i) => {
                                ...
                                return (
                                  <div key={i} className="flex items-center gap-2 text-xs">
                                    ...
                                    <span className="text-foreground flex-1">{fu.title}</span>
                                    {fu.evidence && (
